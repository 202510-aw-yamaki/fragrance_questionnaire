...
公開用整理後、旧レイアウト参照画像と旧実装ディレクトリは本リポジトリに含めない。

## 2026-05-04 Layout Image Completion State

UI再構築済みページは、現在のHTML/CSS/JSを正本として扱う。

これらは未着手のUI再構築タスクではない。
...
2026-05-06追記:
QR依頼一覧は、スタッフ画面では作成可能 / 作成不可 / 発送完了操作の入口を持つ。管理者画面では同じ依頼を監視・集計用に表示し、スタッフ操作ボタンは出さない。期限処理は管理者画面読込時に `process_qr_request_deadlines()` を呼び、3営業日超過、作成可能メール後の再案内、期限切れメールイベント、無効化QRアクセス通知をDB側で処理する。
